...
namespace NeuralNetwork1
{
    ...
    public class Sample
    {
        ...
        /// </summary>
        public Letter actualClass;

        /// <summary>
        /// Распознанный класс - определяется после обработки
        /// </summary>
        public Letter recognizedClass;
        ...
        public Sample(Bitmap b, double[] inputValues, int classesCount, Letter sampleClass = Letter.undef)
        {
            bitmap = b;
            //  Клонируем массивчик
            input = (double[])inputValues.Clone();
            Output = new double[classesCount];
            if (sampleClass != Letter.undef) Output[(int)sampleClass] = 1;

            recognizedClass = Letter.undef;
        ...
        /// </summary>
        public Letter ProcessPrediction(double[] neuralOutput)
        {
            Output = neuralOutput;
            if (error == null)
            ...
            for (int i = 0; i < Output.Length; ++i)
            {
                ...
                if (Output[i] > Output[(int)recognizedClass]) recognizedClass = (Letter)i;
            }
